...
const AltaArticulo = async (body, modelo) => {
  // codigo && descripcion && subCategoriaId && marcaId,talleId
  ...
  if (codigo) {
      articulo = await ModeloArticulo.findAll({
          include: [
              ...
              { model: ModeloSubCategoria, attributes: ["name", "activo","categoriaId"],include:[
                {model:Categorias,attributes:["name","descripcion"]}] },
